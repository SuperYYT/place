...
class DataModelManager {
  ...
  registerModel(name, prototype) {
    ...
    if (needsInitialization) {
      ...
      Object.keys(prototype.statics).forEach((key) => {
        ...
        prototype.statics[key] = function () {
          return m
            ._getMethodHandler(name, key, true)
    ...
    var model =
      ...
      mongoose.model(name, this.registeredModels.get(name) || prototype);
    if (needsInitialization) {
      ...
      if (methodsToAdd)
        Object.keys(methodsToAdd).forEach((key) => {
          this._setupMethodHandler(name, key, false);
        });
    }
    return model;
  ...
  registerModuleOverrides(name, override) {
    ...
    if (!this.modelInstanceMethodOverrides[name])
      this.modelInstanceMethodOverrides[name] = {};
    if (!this.modelStaticMethodOverrides[name])
      this.modelStaticMethodOverrides[name] = {};
    // Loop through the added instance methods, and set them up to work
    Object.keys(override.methods).forEach((key) => {
      if (!this.modelInstanceMethods[name][key])
        this.modelInstanceMethods[name][key] = [];
    ...
    Object.keys(override.hookMethods).forEach((key) => {
      if (!this.modelInstanceMethodOverrides[name][key])
        this.modelInstanceMethodOverrides[name][key] = [];
      this.modelInstanceMethodOverrides[name][key].push(
        override.hookMethods[key]
      );
    });
    Object.keys(override.hookStatics).forEach((key) => {
      if (!this.modelStaticMethodOverrides[name][key])
        this.modelStaticMethodOverrides[name][key] = [];
      this.modelStaticMethodOverrides[name][key].push(
        override.hookStatics[key]
      );
    });
  }
  ...
  _setupMethodHandler(name, key, isStatic) {
    ...
    mongoose.models[name].prototype[key] = function () {
      return m
        ._getMethodHandler(name, key, false)
        .apply(this, Array.from(arguments));
    };
  }
  ...
  _registerMethodHandler(model, name, isStatic, handler) {
    ...
    if (!this._getList(isStatic)[model][name])
      this._getList(isStatic)[model][name] = [];
