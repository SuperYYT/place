...
function LeaderboardManager(app) {
  var manager = {
    ...
    update: function () {
      ...
      var dateBackLastWeek = new Date(
        new Date().getTime() - 7 * 24 * 60 * 60 * 1000
      );
      var pixelCounts = {};
      Pixel.find({ lastModified: { $gt: dateBackLastWeek } }, { editorID: 1 })
        .cursor()
        .on("data", (pixel) => {
        ...
        .on("end", () => {
          ...
          m.topUsers = Object.keys(pixelCounts)
            .map((userID) => [userID, pixelCounts[userID]])
            .sort((a, b) => b[1] - a[1])
            .map((a) => a[0]);
          // Remove banned and deactivated users
          User.find(
            { _id: { $in: m.topUsers } },
            { _id: 1, banned: true, deactivated: true }
          )
            .then((users) => {
              m.topUsers = users
                .filter((u) => !u.banned && !u.deactivated)
                .sort((a, b) => m.pixelCounts[b._id] - m.pixelCounts[a._id])
                .map((u) => u.id);
              m.isUpdating = false;
              ...
              app.logger.log(
                "Leaderboard",
                "Generation of leaderboard data complete."
              );
            })
            .catch((err) => {
              app.logger.capture(
                "Couldn't update leaderboard, removal operation failed: " + err
              );
              m.topUsers = null;
              m.pixelCounts = null;
              m.isUpdating = false;
    ...
    getInfo: function (callback) {
      ...
      if (!this.topUsers || !this.pixelCounts)
        return callback("No leaderboard data loaded", null);
      User.find({ _id: { $in: this.topUsers } })
        .then((users) => {
          var info = {
            leaderboard: users
              .sort((a, b) => this.pixelCounts[b._id] - this.pixelCounts[a._id])
              .map((u) => u.toInfo(app)),
