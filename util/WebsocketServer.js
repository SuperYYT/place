...
function WebsocketServer(app, httpServer) {
  ...
  let obj = {
      ...
      this.server.on("connection", (socket) => {
        ...
        socket.on("fetch_pixels", (ts) => {
          ...
          if (hasRequestedFetch) {
            app.logger.info(
              "Websocket Server",
              "Disconnected client for requesting fetch more than once."
            );
            return socket.disconnect();
          }
          hasRequestedFetch = true;
          var date = new Date(ts * 1000);
          Pixel.find({ lastModified: { $gte: date } })
            .then((pixel) => {
            ...
            .catch((err) => {
              app.logger.capture(
                "Error fetching pixel for websocket client: " + err
              );
            });
        });
      });
    ...
    checkUserCount: function () {
      if (
        !this.lastConnectedClientBroadcastCount ||
