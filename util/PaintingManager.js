...
function PaintingManager(app) {
  ...
  const cachedImagePath = path.resolve(
    app.dataFolder,
    "cached-board-image.png"
  );
  const temporaryCachedImagePath = path.resolve(
    app.dataFolder,
    "cached-board-image.png.tmp"
  );
  return {
    hasImage: false,
    ...
    getStartingImage: async function () {
      try {
        ...
        if (image.bitmap.width != imageSize || image.bitmap.height != imageSize)
          return {
            image: await this.createNewImage(),
            canServe: false,
            skipImmediateCache: false,
          };
    ...
    loadImageFromDatabase: function () {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.getStartingImage()
          .then(async ({ image, canServe, skipImmediateCache }) => {
            if (canServe) {
              app.logger.info(
                "Startup",
                `Got initially serveable image, serving...`
              );
            ...
            Pixel.count({}).then((count) => {
              ...
              var progressUpdater = setInterval(() => {
                app.logger.info(
                  "Startup",
                  `Loaded ${loaded.toLocaleString()} of ${count.toLocaleString()} pixel${
                    count == 1 ? "" : "s"
                  } (${Math.round((loaded / count) * 100)}% complete)`
                );
              }, 2500);
              Pixel.find({})
                .cursor()
                ...
                .on("end", () => {
                  ...
                  app.logger.info(
                    "Startup",
                    `Loaded total ${count.toLocaleString()} pixel${
                      count == 1 ? "" : "s"
                    } pixels from database. Applying to image...`
                  );
                  if (this.pixelsToPreserve)
                    this.pixelsToPreserve.forEach((data) =>
                      image.setPixelColor(data.colour, data.x, data.y)
                    );
                  this.pixelsToPreserve = null;
                  app.logger.info(
                    "Startup",
                    `Applied pixels to image. Serving image...`
                  );
                  serveImage(image);
                })
                .on("error", (err) => {
      ...
      return new Promise((resolve, reject) => {
        if (this.outputImage)
          return resolve({
            ...
            hasChanged: this.imageHasChanged,
            generated: this.lastPixelUpdate,
          });
                ...
                fs.writeFile(temporaryCachedImagePath, buffer, (err) => {
                  if (err)
                    return app.logger.error(
                      "Painting Manager",
                      "Couldn't save cached board image, got error:",
                      err
                    );
                  if (fs.existsSync(cachedImagePath))
                    fs.unlinkSync(cachedImagePath);
                  fs.rename(
                    temporaryCachedImagePath,
                    cachedImagePath,
                    (err) => {
                      if (err)
                        return app.logger.error(
                          "Painting Manager",
                          "Couldn't move cached board image into place, got error:",
                          err
                        );
                      app.logger.info(
                        "Painting Manager",
                        "Saved cached board image successfully!"
                      );
                    }
                  );
                });
    ...
    doPaint: function (colour, x, y, user) {
      ...
      return new Promise((resolve, reject) => {
        if (!this.hasImage)
          return reject({
            message:
              "Our servers are currently getting ready. Please try again in a moment.",
            code: "not_ready",
          });
        if (app.temporaryUserInfo.isUserPlacing(user))
          return reject({
            message: "You cannot place more than one tile at once.",
            code: "attempted_overload",
          });
        ...
        user.addPixel(colour, x, y, app, (changed, err) => {
          ...
          const pixelData = {
            x: x,
            y: y,
            colour: Jimp.rgbaToInt(colour.r, colour.g, colour.b, 255),
          };
          ...
          var info = {
            x: x,
            y: y,
            colour: Pixel.getHexFromRGB(colour.r, colour.g, colour.b),
          };
      ...
      setInterval(() => {
        if (this.pixelsToPreserve)
          return app.logger.log(
            "Painting Manager",
            "Will not start board image update, as board image is still being completely loaded..."
          );
        if (this.isGenerating)
          return app.logger.log(
            "Painting Manager",
            "Will not start board image update, as board image is still being generated..."
          );
        if (!this.imageHasChanged)
          return app.logger.log(
            "Painting Manager",
            "Not updating board image, no changes since last update."
          );
